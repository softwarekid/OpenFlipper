...
int ManifoldVertex::global_id = 0;
...
BoundingBox ManifoldEdge :: getBoundingBox() const
{
...
BoundingBox ManifoldFace :: getBoundingBox() const
{
...
int
intersect(const ManifoldEdge *seg1, const ManifoldEdge *seg2, Point2D &point, double &sab, double &scd)
{
                ...
                vector<int> &faceSides, vector<ManifoldVertex*> &newnodes)
{
    ...
    for (int i = 0; i < numSegments; i++) {
        const ManifoldVertex *v0 =  face->getNodeAt( (i+0)%numSegments);
        const ManifoldVertex *v1 =  face->getNodeAt( (i+1)%numSegments);
        ManifoldEdge  faceedge(v0,v1);
...
bool GeomCurve :: isChain() const
{
    ...
    const ManifoldVertex *prev_end_vertex;

    int index = 0;
    for( it = edges.begin(); it != edges.end(); ++it) {
        const ManifoldEdge *e = *it;
        if( index++ == 0 ) {
...
ManifoldEdge* GeomCurve :: splitAt( ManifoldEdge *edge, ManifoldVertex *vmid)
{
    //
    ...
    list<ManifoldEdge*>::iterator it;
    for( it = edges.begin(); it != edges.end(); ++it)
        if( *it == edge ) break;

    assert( it != edges.end() );

    const ManifoldVertex *v0 = edge->getNodeAt(0);
    const ManifoldVertex *v1 = edge->getNodeAt(1);

    edge->setNodes( vmid, v1 );

    ManifoldEdge *newedge = new ManifoldEdge(v0, vmid);
...
///////////////////////////////////////////////////////////////////////////////
bool ManifoldFace::contains(Point2D &pnew) const
{
    double  ori[3];
...
ManifoldFace *FaceMesh::contains( const ManifoldVertex *v) const
{
    assert(v);
...
void FaceMesh :: build_relations00()
{
    ...
    for( int i = 0; i < numfaces; i++) {
        ManifoldFace *face = faces[i];
        int nnodes = face->getSize(0);
        for( int j = 0; j < nnodes; j++) {
            ManifoldVertex *v = face->getNodeAt(j);
            v->clearRelations(0);
        }
    }
    ...
    for( int i = 0; i < numfaces; i++) {
        ...
        int nnodes = face->getSize(0);
        for( int j = 0; j < nnodes; j++) {
            ManifoldVertex *v0 = face->getNodeAt((j+0)%nnodes);
            ManifoldVertex *v1 = face->getNodeAt((j+1)%nnodes);
            v0->addRelation(v1);
            v1->addRelation(v0);
        }
    }
...
void FaceMesh :: build_relations02()
{
    ...
    for( int i = 0; i < numfaces; i++) {
        ...
        int nnodes = face->getSize(0);
        for( int j = 0; j < nnodes; j++) {
            ManifoldVertex *v0 = face->getNodeAt(j);
            v0->addRelation(face);
        }
    }
}
...
vector<ManifoldEdge*>  FaceMesh :: getEdges()
{
     build_relations00();

     vector<ManifoldEdge*>  meshedges;

     size_t numnodes =  getSize(0);
     meshedges.reserve( 3* numnodes );
     for( int i = 0; i < numnodes; i++) {
          ManifoldVertex *src = nodes[i];
          const vector<ManifoldVertex*> &neighs = src->getRelations0();
          for( int j = 0; j < neighs.size(); j++) {
               ManifoldVertex *dst = neighs[j];
               if( src < dst ) {
                   ManifoldEdge *newedge = new ManifoldEdge( src, dst );
...
vector<ManifoldFace*> FaceMesh :: getRelations112( const ManifoldVertex *v0, const ManifoldVertex *v1)
{
    const vector<ManifoldFace*> &v0faces = v0->getRelations2();
    const vector<ManifoldFace*> &v1faces = v1->getRelations2();

    assert( !v0faces.empty()  && !v1faces.empty() );

    vector<ManifoldFace*> faceneighs;
...
int NonManifoldMesh2D :: readEmbeddedTriMesh( const string &filename)
{
    ...
    for( size_t i = 0; i < numNodes; i++) {
        ...
        ManifoldVertex *v = new ManifoldVertex;
        v->setPristine(1);
        v->setID(ManifoldVertex::global_id++);
        v->setCoords(p);
        mesh->nodes[i] = v;
    }

    int n0, n1, n2, nSides;
    vector<ManifoldVertex*> connect(3);
    int minid = std::numeric_limits<int>::max();
    for( size_t i = 0; i < numFaces; i++) {
        infile >> nSides >> n0 >> n1 >> n2;
        assert( nSides == 3 );
        ManifoldFace *face = new ManifoldFace;
...
///////////////////////////////////////////////////////////////////////////////
int NonManifoldMesh2D :: addEmbeddedTriMesh ( const vector<ManifoldFace*> &faces)
{
    mesh = new FaceMesh;
    mesh->faces = faces;
    return 0;
...
GeomCurve* NonManifoldMesh2D :: addCutCurve( vector<ManifoldVertex*> &nodes,
        int closedCurve)
{
    ManifoldVertex *v0, *v1;
    ManifoldEdge  *segment;
    ...
    */
    for( size_t i = 0; i < numNodes-1; i++) {
        v0 = nodes[i];
        v1 = nodes[i+1];
        segment = new ManifoldEdge( v0, v1);
...
{
    ManifoldFace *face = NULL;
    ManifoldEdge *edge = NULL;
...
//CSV
    ...
    vector<ManifoldVertex*>  newnodes;
    while( it != curve->edges.end() ) {
        ...
        if( newnodes.size() ) {
            cout << " Intersect Now " << ManifoldVertex::global_id << endl;
            for( int i = 0; i < newnodes.size(); i++) {
                newnodes[i]->setID( ManifoldVertex::global_id++);
...
void NonManifoldMesh2D :: enumerate()
{
    ...
    for( int i = 0; i < numFaces; i++) {
        ManifoldFace *face = mesh->faces[i];
        for( int j = 0; j < 3; j++) {
...
void NonManifoldMesh2D :: saveAs( const string &filename)
{
    ...
    for( int i = 0; i < numFaces; i++) {
        ManifoldFace *face = dupmesh->faces[i];
